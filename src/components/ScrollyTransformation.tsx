			...
			<aside className="scrolly-sticky">
				<figure className="figure-plate relative m-0">
					...
					<div className="flex items-center justify-between gap-3">
						<p className="kicker">How an engagement works</p>
					...
					<div className="mt-4 flex gap-1.5" aria-hidden>
						{steps.map((step, i) => (
							<span
								...
								style={{
									background:
										i <= activeIndex ? "var(--violet)" : "var(--border)",
								}}
							/>
						))}
					</div>
					<div className="mt-5 flex items-baseline gap-3">
						<span className="font-display text-2xl font-medium text-violet">
							{`0${activeIndex + 1}`}
						</span>
					...
					<div className="mt-5 rounded-[14px] border border-border bg-canvas-2 p-4">
						<p className="kicker">What you get</p>
						<p className="mt-2 text-sm text-ink-soft">{active.outcome}</p>
					</div>
				</figure>
			</aside>
